...
export default function AnalyticsPage() {
  ...
  async function loadAnalytics(showLoader = false) {
    try {
      ...
      setError(
        err?.message || "Failed to load analytics from Google Sheets"
      );
  ...
  const attendanceRate =
    summary.totalDays > 0
      ? Math.round(
          (summary.totalActiveDays / summary.totalDays) * 100
        )
      : 0;
    ...
    >
      <section className="rounded-2xl border border-cyan-300/15 bg-[#071018]/80 p-5 backdrop-blur-xl">
        <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Weekends and admin holidays are excluded.
              EntryTime controls late and half day.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                className="flex items-center gap-2 rounded-xl border border-white/10 bg-black/25 px-3 py-2.5 text-sm font-semibold text-slate-300 transition hover:border-cyan-300/25 hover:text-white"
              >
                <ChevronLeft size={16} />
                Previous Month
              </button>

              <div className="min-w-[180px] rounded-xl border border-cyan-300/15 bg-cyan-300/[0.05] px-4 py-2.5 text-center">
              ...
              <input
                ...
                className="rounded-xl border border-white/10 bg-black/25 px-3 py-2.5 text-sm text-white outline-none focus:border-cyan-300/35"
              />

              <button
                onClick={showCurrentMonth}
                disabled={isCurrentMonth}
                className="flex items-center gap-2 rounded-xl border border-cyan-300/20 bg-cyan-300/10 px-3 py-2.5 text-sm font-bold text-cyan-200 transition hover:bg-cyan-300/15 disabled:cursor-not-allowed disabled:opacity-40"
              >
                <RotateCcw size={15} />
                Current Month
              </button>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          ...
          <SalaryMiniCard
            ...
            value={
              loading
                ? "..."
                : formatPKR(summary.totalCommission)
            }
          ...
          <SalaryMiniCard
            ...
            value={
              loading
                ? "..."
                : formatPKR(summary.totalDeductions)
            }
            icon={TrendingDown}
            tone="text-red-300"
          />
                  ...
                  >
                    <td className="px-4 py-4">
                      ...
                      <p className="text-xs text-slate-500">
                        Entry {agent.entryTime} · Base{" "}
                        {formatPKR(agent.baseSalary)}
                      </p>
        ...
        <div className="mt-4 rounded-2xl border border-yellow-300/15 bg-yellow-300/[0.04] px-4 py-3 text-xs leading-relaxed text-slate-400">
          Salary rule: Late = daily salary / 3. Half day =
          daily salary / 2. Absent = full daily salary.
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
      <div className="mb-3 flex items-center justify-between">
        <div
          className={`rounded-xl bg-white/5 p-2 ${tone}`}
        >
          <Icon size={17} />
        </div>
      </div>
